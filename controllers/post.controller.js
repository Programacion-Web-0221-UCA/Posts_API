...
controller.toggleLike = async (req, res, next) => {
  try{
    ...
    const { status: postExists, content: post } = await postService.findOneRaw({_id: id, active: true});
    if (!postExists) return res.status(404).json({ error: "Post not found" });
...
controller.toggleFav = async (req, res, next) => {
  try{
    ...
    const { status: postExists } = await postService.findOneRaw({_id: id, active: true});
    if (!postExists) return res.status(404).json({ error: "Post not found" });
